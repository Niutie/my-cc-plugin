...
|---|---|---|
| Phase 1.1-1.2 内容编写 | `references/SCRIPT-STYLE.md` + `references/OUTLINE-FORMAT.md` + `article.md`（用户原文，如有） | `references/TRAINING-CENTER.md`（**做培训 / 认证 / 赋能课视频时必读** —— 产品无关的 L0–L3 分层框架 + 总览/单模块两种粒度） |
...
**在生成 `script.md` 之前先和用户确认本次成片用哪种语言**——语言一旦定了
会贯穿 script / narration / 主题文案 / TTS 音色，回头改成本高。

- **默认跟随原文**：中文文章 → 中文成片，英文文章 → 英文成片（与
  [`SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md) 的"保持原文语言"底线一致）。
- 先**判定原文主语言**，再用一句话**显式确认 + 给反悔机会**，例如：
  > 这篇是中文，我就做**中文**成片（口播 + 字幕 + 音色都中文）。要做**英文**版吗？
- 用户要求**跨语言**（如中文文章做英文视频）→ 确认后按**目标语言**写 script
  和 narration（这属于 SCRIPT-STYLE "明确要求翻译"的情形），TTS 也选对应
  语言音色（edge-tts/openai/minimax 传对应 voice）。
...
**默认是娱乐 / 解说向（B 站基调）。但如果用户要的是培训 / 认证 / 赋能课
视频（或明说"training center / 培训"）→ 切到「培训中心」体裁**——这是
**结构驱动**的体裁，会改变 script + outline 的组织方式，必须在动笔前定。
...
1. **生成 `script.md`**：按 [`references/SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md)
   的规则把 article 转成平台化口播稿，**用 1.1.5 确认的成片语言**（默认跟随
   原文）。**保留 `article.md` 不删**——它是
...
4. **若走的是培训中心体裁**（§1.1.6）：把 `training-center` 主题列为**推荐
