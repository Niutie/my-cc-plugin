...
description: 把一篇文章或口播稿，做成"看起来像视频"的点击驱动 16:9 网页演示，可选合成口播音频。流程：原始文章 → **先与用户确认成片语言（中文 / 英文，无论原文中 / 英都要问）** → **默认体裁 = 培训中心（结构驱动，L0–L3；用户明确要解说 / 娱乐向才切）** → **一次产出**口播稿 + outline 开发计划 → 用户**一次对齐** 5 件事（稿子 / outline / 主题 / 素材 / 开发模式）→ 网页开发（逐章 / 顺序 / 并行）→ 可选音频合成（provider-agnostic：内置 MiniMax mmx-cli + OpenAI TTS + edge-tts（免费 / 无 key），可换 ElevenLabs / Azure / 自带 TTS）。**outline 只规划节奏与信息密度，不规划动画** —— 动画由章节开发时按 PRINCIPLES + ANTI-AI 法则即时设计。每次点击推进口播稿的一个节拍，每一步独占整屏，进度条平时隐藏只在悬浮时出现。适用场景：用网页做视频（动态 PPT 但不像 PPT）、把口播稿 / 文章变成可交互的解说、为 B 站 / YouTube / 视频号录屏教程、做有电影感的产品 / talk demo、做企业培训 / 认证 / Partner 赋能课程视频（培训中心体裁，按 L0–L3 分层）。本 plugin 沉淀的是设计方法论 + 协作流程 —— 不绑定任何特定样式 / 字体 / 颜色 —— 因此能复用到任意主题与美学。
---
...
本 plugin **以方法论 + 协作流程为核心**。脚手架模板提供 token 和原语，
...
```
Phase 1   内容编写
   1.1  识别用户输入 + 确认成片语言（中/英，无论原文语言都要问）
        + 确认体裁（默认培训中心，用户明确要解说 / 娱乐向才切）
...
└── presentation/       # 脚手架产出的 Vite + React + TS 项目
    ...
    │   ├── record-video.mjs        # ★ 一键出片：无头驱动 ?capture=1 + ffmpeg 合轨 → mp4
    │   └── tts-providers/          # 每 provider 一个 .sh（内置 3 个）
    │       ├── README.md           # 三函数契约 + 4 段现成代码片段（11labs / say / azure / gcloud）
    │       ├── minimax.sh          # 默认 provider，用 mmx-cli（收费）
    │       ├── openai.sh           # 内置 OpenAI TTS（curl + OPENAI_API_KEY，收费）
    │       └── edge-tts.sh         # 内置免费 provider（pip install edge-tts，无 key）
...
|---|---|---|
| Phase 1.1-1.2 内容编写 | **`references/TRAINING-CENTER.md`（默认体裁必读 —— 产品无关 L0–L3 分层框架 + 总览/单模块两种粒度）** + `references/SCRIPT-STYLE.md`（信息保留度 + 去 AI 味通用层，对所有体裁适用）+ `references/OUTLINE-FORMAT.md` + `article.md`（用户原文，如有） | 用户明确要**解说 / 娱乐向**时 → SCRIPT-STYLE.md 的 8 形式原则 + 平台变体表 |
...
| Phase 3 音频合成 | `references/AUDIO.md`（含 narrations.ts → segments.json → 任意 provider 流程，内置 minimax + openai + edge-tts 免费） | `templates/scripts/tts-providers/README.md`（换 provider / 自带 TTS 时） |
| Phase 4 出片 | `references/RECORDING.md`（① 一键出片 `npm run record-video` / ② `?auto=1` 手动录屏 / ③ Manual） | —— |
...
| 啥都没有，只说"帮我做个 X 主题的视频" | **反问**：先给一段素材或大纲。plugin 不替用户构思内容 |

### 1.1.5 确认成片语言（产出 script.md 前必做）

**在生成 `script.md` 之前必须和用户确认本次成片用哪种语言 —— 无论原文是
中文还是英文都要问，不要替用户预设"跟随原文"**。语言一旦定了会贯穿
script / narration / 主题文案 / TTS 音色，回头改成本高。

- `--lang=zh|en` 给了就当作已确认，直接用、不再追问。
- 没给 → **判定原文主语言只用于给提示，仍要把"中文 / 英文"作为一个明确
  选择问出来**，不要预设答案。例如：
  > 这篇原文是中文。成片你要做**中文**还是**英文**？（决定口播稿 + TTS 音色 + 章节屏幕文案的语言）
- 用户选**跨语言**（如中文文章做英文片）→ 按**目标语言**写 script 和
  narration（这属于 SCRIPT-STYLE "明确要求翻译"的情形），TTS 选对应语言
  音色（edge-tts/openai/minimax 传对应 voice）。
- 一句脚本里中英混排的术语 / 产品名 / 代码照原文保留，**不要硬翻**。

> 确认到的语言记一下，Checkpoint Plan 的总结里要带出来（也是 Phase 3 选
> TTS 音色的依据）。

### 1.1.6 确认内容体裁（产出 script.md 前必做）

**默认体裁是「培训中心」（结构驱动的赋能 / 认证课基调）。** 用户带来素材
但没特别说体裁时，**按培训中心走**：目标先行、L0–L3 分层、能力边界 + 阶段
小结。只有用户**明确要解说 / 娱乐向**（B 站 / 抖音 / YouTube 等，或明说
"做个解说 / B 站风"）时，才切到解说向体裁。体裁是**结构驱动**的、会改变
script + outline 的组织方式，**必须在动笔前定**。

**默认 · 培训中心体裁**（不确定时走这个）：

1. **先读** [`references/TRAINING-CENTER.md`](references/TRAINING-CENTER.md)
   （产品无关的 L0–L3 分层框架 + 风格约定）。
2. **确认粒度**：这条视频是「课程总览 / 导学」还是「单模块深讲」（两者章节
   结构不同，见 TRAINING-CENTER.md）。
3. 按 L0–L3 框架写 script + outline；**目标先行 + 能力边界 + 阶段小结**。
4. **视觉默认推荐 `training-center` 主题**（Checkpoint Plan 选主题时优先荐它）。

> **🔒 铁律**：示例产品（如某截图里的 API 网关）只是用来让你理解骨架 ——
> **绝不写进任何 script / outline / 章节**。用户带来什么产品就填什么产品。

**opt-in · 解说 / 娱乐向体裁**（用户明确要时才切）：按
[`references/SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md) 的 8 条形式原则 +
平台变体（B 站 / 抖音 / YouTube / 知乎 / 小红书）写 —— 钩子、短句、段子、
数字翻译成感受。**切了要告诉用户你按解说向写**，给反悔机会。

> 无论哪种体裁，**信息保留度 ≥ 60% + 去 AI 味五类 + 念出来测试**（见
> SCRIPT-STYLE.md 通用层）都适用、都要过。
...
1. **生成 `script.md`**：按 [`references/SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md)
   的规则把 article 转成平台化口播稿，**用 1.1.5 确认的成片语言**（§1.1.5
   已显式确认，不预设跟随原文）。**保留 `article.md` 不删**——它是
...
4. **默认（培训中心体裁，§1.1.6）**：把 `training-center` 主题列为**推荐
   第一**，并在总结里额外带出本条视频的**粒度**（课程总览 / 单模块深讲）；
   仅当用户切了解说 / 娱乐向，才按 script 内容从其它主题推荐

### 总结模板（骨架，agent 按情况填充）

```
内容计划写完，产出文件：
  🌐 成片语言       {中文 / 英文}（已确认，决定口播稿 + TTS 音色 + 章节屏幕文案语言）
...
> ```
>
> 然后让用户开 `localhost:5174/?auto=1` → SPACE 听，重点抓「某段 ≥15s = 文案太密 /
...
要不要合成音频做"自动播放录屏"？
  ✓ 合成 → 扫所有章节的 narrations.ts 出 audio-segments.json，
           ...
           内置三个 provider：
             • minimax (mmx-cli)    —— 默认，中文音色稳（收费）
             • openai  (OPENAI_API_KEY) —— curl-based，多数已有 key（收费）
             • edge-tts (pip install edge-tts) —— 免费 / 无 key，中英都行
           其它后端 (ElevenLabs / macOS say 离线 / Azure / Google) 见
           scripts/tts-providers/README.md 的现成片段。
...
详见 [`references/RECORDING.md`](references/RECORDING.md)。三条路径，从省事到费事：

| 场景 | 推荐路径 |
|---|---|
| Phase 3 已合成音频 | **① 一键出片（首推）**：`cd presentation && npm run record-video` → 无人值守直接出 `output/video.mp4`。无头驱动 `?capture=1` 录画面、把每步 mp3 按同一时刻表拼回音轨、ffmpeg 合成，**音画天然同步、不用手动录屏 / 不用后期对轨**。要 ffmpeg + ffprobe + 一个浏览器（默认系统 Chrome）。等价命令 `/video-maker:capture` |
| 想自己掌控录制 | **② Auto 一镜到底**：浏览器开 `localhost:5174/?auto=1` → 按 SPACE → 整片自动播完 → 自己用屏幕录制软件录 → 裁头尾即成片 |
| Phase 3 跳过 | **③ Manual**：手动点击推进 → 后期任意剪辑工具配音（或先跑 ① 出一版静音估时片预览） |

> agent 在 Phase 3 / Checkpoint Audio 后**主动告诉用户**适合的出片路径：合成了音频
> 就首推 ① 一键出片（`npm run record-video`）。
...
| 文件 | 何时读 | 内容 |
|---|---|---|
| [`references/SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md) | Phase 1.2 必读 | 文章 → 口播稿规则、平台变体（含培训中心入口） |
| [`references/TRAINING-CENTER.md`](references/TRAINING-CENTER.md) | 培训 / 认证 / 赋能课题材时（§1.1.6） | 产品无关的 L0–L3 分层框架、课程总览 vs 单模块两种粒度、中英双语 + 时长预算约定、反模式 |
...
| [`references/AUDIO.md`](references/AUDIO.md) | Phase 3 才读 | provider-agnostic 音频合成流程、内置 minimax / openai / edge-tts(免费) 用法、换 provider 路径、故障排查 |
| [`templates/scripts/tts-providers/README.md`](templates/scripts/tts-providers/README.md) | 换 / 加 TTS provider 时 | 三函数契约 + 内置 3 个 (minimax / openai / edge-tts) + 4 种现成代码片段（ElevenLabs / macOS say / Azure / Google） |
| [`references/RECORDING.md`](references/RECORDING.md) | Phase 4 才读 | ① 一键出片（`npm run record-video` 无头 → mp4）/ ② 手动录屏 / ③ Manual + 后期 |
