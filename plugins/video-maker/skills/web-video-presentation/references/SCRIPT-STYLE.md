...
把书面文章转成"说出来不别扭"的口播稿。本指南分两层：**通用层**（信息保留度
≥ 60% + 去 AI 味五类 + 念出来测试）对**所有体裁**适用；**形式层**（下面 8 条
原则 + 末尾平台变体）是**解说 / 娱乐向**（B 站 / 抖音 / YouTube 等）的写法。

> **默认体裁是「培训中心」**（结构驱动，目标先行 / L0–L3，见
> [`TRAINING-CENTER.md`](TRAINING-CENTER.md)）。只有用户**明确要解说 / 娱乐向**
> 时才套用下面 8 条形式原则与平台变体；通用层两节对培训中心同样适用。
...
>    一耳朵是机器。**口播比文字更怕 AI 味**。
> 3. **用 1.1.5 确认的成片语言（不要擅自改语言）** —— 成片语言在
>    Phase 1.1.5 已与用户**显式确认**（无论原文中 / 英都要问，不预设
>    "跟随原文"）。`script.md` 必须用确认的那个语言：选了保持原文就保持，
>    选了跨语言（如中文文章做英文片）就整体按目标语言写。
>    中英混排时保留原文中的术语、引用、产品名、代码和专有名词。
>    "解说 / B 站 / YouTube 风格 / 口语化"只调整节奏、结构和表达自然度，
>    **不改变语言**。
>
...
|---|---|---|---|---|
| **B 站** | 自来熟、有段子、节奏快 | ≤ 20 字 | 3 秒钩子 | 中（5 秒/idea） |
...
| **培训中心 / Training Center**（默认体裁） | 专业、带教、目标先行 | ≤ 25 字（清晰完整） | 不用钩子，开头点学习目标 | 高（结构化分层） |

> **小红书闺蜜感 ≠ 假共情**。"姐妹们听我说，这个真的踩雷了" 是闺蜜
> 感（具体 + 有情绪）；"我知道你最近肯定很纠结" 是 AI 假共情（空泛
> + 上来就装熟）。区别在**有没有具体场景**。
>
> **培训中心是"结构驱动"的体裁，不只是语气微调** —— 它把产品按 L0–L3
> 能力分层组织成赋能 / 认证课程，目标是让学习者"学完能做某事"。完整框架
> （L0–L3 分层定义 / 课程总览 vs 单模块深讲两种粒度 / 中英双语 + 时长预算
> 约定 / 反模式）见 [`TRAINING-CENTER.md`](TRAINING-CENTER.md)；视觉配套
> `training-center` 主题。
...
### 2. 形式层自检（解说 / 娱乐向：8 条原则）

> 走**培训中心**（默认体裁）时，这一层换成 TRAINING-CENTER.md 的结构自检
> （目标先行 / 能力边界 / L0–L3 分层 / 阶段小结）；下面 8 条是解说向才用。
> 通用层（信息保留度 + 去 AI 味 + 念出来）对两种体裁都要过。
