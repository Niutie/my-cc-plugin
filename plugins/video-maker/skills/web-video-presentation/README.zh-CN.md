...
- **可插拔 TTS**：provider-agnostic 音频 runner，**内置 3 个 provider**（MiniMax `mmx-cli` + OpenAI TTS via curl，均收费；外加 **edge-tts 免费 / 无 key**）；往 `tts-providers/` 丢一个 `.sh` 就能换成 ElevenLabs / Azure / Google Cloud / macOS `say` / 任何自部署 TTS。
...
```text
Phase 1.1  识别用户输入 + 确认成片语言（中/英，无论原文语言都要问）+ 体裁（默认培训中心）
Phase 1.2  文章 -> 口播稿（按确认的语言）
   |
