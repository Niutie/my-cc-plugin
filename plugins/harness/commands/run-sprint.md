...
你是这个循环的**主 orchestrator**。当用户触发 `/run-sprint`，你必须按以下手册顺序执行，直到 `sprint-status.yaml` 中再无 `backlog` story 为止。**禁止并行**多条 story；同一时刻最多一条 story 在流水线里。
...
**测试 harness 独立 stage**（由 `/run-test-sprint` 触发，与 run-sprint 5-stage 命名空间隔离；详见 [`run-test-sprint.md`](run-test-sprint.md)）：
...
- **review 基线**：用 `harness-state.py` 拿 `stage2_base_sha`，必要时自动补打 tag：
  ...
  > **边界情况：harness 元修改 commit 落在 stage ② 之后**：如果 stage ② → ⑤ 之间因为流水线 halt → 用户授权 harness 元修改 → 重新继续，期间插入了任何 `.claude/commands/run-sprint.md` / `.claude/harness/scripts/**` / `.claude/harness/answer-policy.md` 的元修改 commit——**绝对不要**前推 `harness/$KEY/stage2-base` tag。前推 tag 会让 stage ⑤ bmad review 把 stage ② 的所有代码（往往是 story 的主体实现，几千行）排除出 review window，造成"伪 review 通过"的静默失败（曾在 `1-5-console-web-scaffold-design-tokens-i18n-shadcn` 这条 story 上发生过：bmad 只看到了 290/9941 行，2.9% 覆盖率）。正确做法：**让 tag 留在原位（指向 stage ① commit），review prompt 用 git pathspec `:!.claude` 把 harness 路径的改动排除出 diff**。stage ③ codex 和 stage ⑤ bmad 的 prompt 模板里已经写好了这个 pathspec。
- 调度 dev：
...
阶段 ⑤ commit 完成（sprint-status `<KEY>` status=done）后、阶段 ⑥ 触发判断之前，自动调用 `/run-test-sprint` 跑本 story 的 atdd + e2e。设计意图：把测试纳入 run-sprint 主流水线，闭合"review 过仍出问题"的结构性 gap（详 chore-test-harness-bootstrap spec）。

> **2026-05-04 commit 路径统一（chore-harness-epic-4-orchestration-observations T2.1）**：本 stage 已改为 **review-only** —— `/run-test-sprint` subagent 内部走 T3 + T4 双 commit（颗粒度更细 + atdd 红相 vs e2e 绿相分阶段诊断）；run-sprint 主 agent 不再调用 5-5 commit 当 commit 路径，只做 (a) spawn → (b) 等返回 → (c) 验收产物 → (d) 跑 5-5 期待 STATUS=skip 用作 sanity gate。详 spec 段 Q3 RESOLVED 决策。
>
> **触发评估在 /run-test-sprint 内部完成**：`/run-test-sprint --story $KEY` 启动头部会调 `.claude/harness/scripts/eval_test_stage_triggers.sh` 读 `.claude/harness/test-stage-triggers.yaml` + `.claude/harness/harness-project-config.yaml` 自决跑哪些 stage（T1/T3/T4 + 进阶 T5/T6/CI/test-review）。run-sprint 主流程不传额外参数，也不预先解析触发条件——阶段 ⑤.5 仅负责调用入口 + 5-5 sanity gate。详 [`run-test-sprint.md`](run-test-sprint.md) §0.0.5。
...
1. **spawn run-test-sprint subagent**（subagent 内部跑 §0.1 env probe + §1 T1/T3/T4 + 各自 commit）：
   ...
   > 请直接调用 /run-test-sprint --story $KEY。这是 run-sprint stage 5.5 自动触发，必须以 non-interactive 模式运行（任何 <ask> 节点都不要发问，按 `.claude/harness/answer-policy.md` 自决）。
   >
