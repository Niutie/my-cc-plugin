...
| 单条 story 的**主阶段（①-⑤）**累计调用 > 6 个子 agent（构成：①②③④⑤ 共 5 个 + 至多 1 次中断重启）。阶段 ⑤.5 run-test、⑥ retro、⑥.5 residue、§0.A.0 retro-fulfill 的子 agent **不计入**本预算，但各有自己的上限：⑤.5 每 story 至多 1 个、⑥ / ⑥.5 每 epic 各至多 1 个（各自另允许至多 1 次配额中断重启）、§0.A.0 每个待兑现 dev 项至多 1 个——超过各自上限同样按本行 halt | halt |
