...
2. **看状态**：用 `python3 .claude/harness/scripts/sprint-status.py status <key>` 检查 sprint-status 是否按预期推进——**只读校验**。发现与预期不符**不要**手工 `set`：状态推进由 `harness-commit.py` 在各 stage commit 内 auto-sync 兜住（现协议，见 §0.5 说明），主 agent 记录不一致即可；只有脚本自身 IO 失败（STATUS=halt + REASON=sprint-status auto-sync failed）才 halt。

**关于结构化校验 vs 文本扫描**：早期版本还要在子 agent 返回文本里搜"广义错误词"（`error` / `failed` / `cannot` / `无法`）触发 halt。实测 false positive 极高（"我加了 error handling"/"测试没 fail"等正常完成描述都会命中），且**真正的失败一定能被结构化校验抓到**：产物缺失 → 验收 halt；schema 不合规 → `harness-commit.py` 退出码 1；漏 stage 推进 → harness-commit auto-sync 自动补上（只读状态校验可见）；patch 没改对 → bmad/codex review 这一轮就会发现。所以从 §K（2026-05-01）起，**主 agent 不再做广义错误词文本扫描**——信任结构化校验链。

**例外：配额耗尽**。runtime quota 信号在结构化层不可见（agent 退出时只是返回一段 "hit your limit ... reset at HH:MM" 的英文）。这一类**仍然要扫文本** → 命中即 halt + 用 §3 配额专属模板（选项 5：等到 reset 时间再让用户决定是否重启 stage）。匹配关键词：`hit your limit` / `rate limit` / `usage limit` / `quota` / `reset` 时间提示。**stage 3 例外**：阶段 ③（codex review）的 quota/auth 命中**不 halt**，改走 §③.1 skip+marker 路径（catchup-friendly，见 §③.1/§③.2）；其余 stage 仍按本条 halt。
...
**(b) 代答政策（重要）**：用户授权你在循环中替他回答所有 BMad / Codex 子 agent 的问询。代答政策的通用决策原则全文在 `.claude/harness/answer-policy.md`（跨项目通用；项目特定语境由 `harness-prompt-suffix.py` 从 harness-project-config.yaml 读出并内联进 prompt 后缀）。
...
1. **每一个** subagent prompt 末尾都必须附带"代答政策附带段"。**不要**手贴文本——一处漏粘就让子 agent 走偏。改用：
   ...
   > **代答政策**：本次任务以非交互模式运行。请先 Read `.claude/harness/answer-policy.md`（跨项目通用流程决策原则），按其中的决策原则 + 下面的项目语境自决，不要发问。每个非显然的选择都要把理由写进你最终交付的产物里。
   >
   > （脚本输出在这句之后还会内联"**项目语境**"段——来自 harness-project-config.yaml `extra.project_context`。）
...
| `--epic` | `single-epic` | 先跑 `python3 .claude/harness/scripts/sprint-status.py next` 拿下一条 backlog：退出码 0 → `TARGET_EPIC = $(python3 .claude/harness/scripts/sprint-status.py epic-of <next>)`，走 §0.A；退出码 1（无 backlog）→ 扫所有 epic（按 yaml 出现顺序），找最早一个 `epic-N-retrospective` 状态非 `done` 的 epic，`TARGET_EPIC = N`，**再跑 `epic-all-done N` 验证**（"无 backlog" ≠ "epic 全 done"——story 可能停在 review / in-progress）：退出码 0 → 走 §0.C retro-only；非 0 → halt（既有 §3 模板），提示先 `/harness-zh:run --continue` 续作该 epic 的中断 story，**不要**对带半成品 story 的 epic 直接 retro；都没找到 → 直接退出回报"sprint 已全部完成" | §0.A 或 §0.C 或直接退出 |
...
1. 取本轮第一条 backlog story key（`all`：`python3 .claude/harness/scripts/sprint-status.py next`；`single-epic`：`python3 .claude/harness/scripts/sprint-status.py next-in-epic $TARGET_EPIC`——**不要**用全局 `next`，目标 epic 之前的其它 epic 可能仍有 backlog，会取错 key；`single-story`：`TARGET_KEY`）。
...
1. **实现**：spawn 一个 fresh general-purpose subagent，prompt = `$SPEC` 全文 + 一句指令"按本 chore spec 的 Tasks & Acceptance 段实现项目代码；这是一次性 chore 兑现，**不走** atdd / codex / bmad review 循环；完成后在 spec 的 Tasks checkbox 打勾。只动 spec 描述范围内的项目代码，不碰其它 story 的 artifacts；spec frontmatter 若声明了 `planning_artifacts:` 白名单（issue #9），按 spec 要求回写那些 planning-artifacts 文件也在 scope 内。" 末尾附 `python3 .claude/harness/scripts/harness-prompt-suffix.py 2`（代答政策段；用 stage 2 的 dev 口径）。
...
2. 设 `KEY`（阶段 ⑥/⑥.5/6-done 的 commit 位参必填；retro-only 路径没有"当前 story"，统一取**该 epic 最后一条 done story 的 key**）：用 Read 打开 `_bmad-output/implementation-artifacts/sprint-status.yaml` 的 `development_status:` 段，在属于 epic ${EPIC} 的 story key（key 以 `${EPIC}-` 开头）中取按 yaml 出现顺序**最后一条** status=done 的 key。绑定后用 `python3 .claude/harness/scripts/sprint-status.py epic-of $KEY` 验证输出 = `$EPIC`（不符 → 重新取，不要带着错 key 进阶段 ⑥；**禁止**传空串或自己编造 key——`harness-commit.py` 的 key 是必填位参，空串会让 commit 被静默 skip）。
3. 跳过 §1 阶段 ①-⑤，直接执行 §1 阶段 ⑥ 的所有动作（条件触发判断改为"无条件触发"——本路径的所有入口（§0.0 `--epic` / `--epic <num>` 行、§2 主循环无 backlog 分支）都已预先用 `epic-all-done` + `epic-retro-status` 验证过 epic 全 done + retro 未 done）。retro subagent 的 prompt 同样必须带阶段 ⑥ prompt 模板里的**格式契约段**（Read bmad-retrospective-suffix.md + Form 1 + Category + Pre-retro self-audit——见阶段 ⑥），不要因为是 retro-only 路径就省略。
...
| 阶段 ② `story($KEY): initial implementation` | 项目代码（除去 §−1.d 黑名单）+ `$KEY.md`（dev 改 status 段）+ `$KEY.dev-result.json`（机器可读完成门）+ `deferred-work.md`（dev 阶段实现中遇到的沙箱延后项 / FU-* 条目）+ `sprint-status.yaml`（harness-commit auto-sync → review） |
...
| 阶段 ⑤ `story($KEY): final review fixes & done` | 项目代码 + `$KEY.md`（含 Status / Review Findings 段）+ `$KEY.review-findings.json`（机器可读完成门）+ `$KEY.review-progress.json`（断点续作 progress 文件，可选）+ `deferred-work.md` + `sprint-status.yaml`（harness-commit auto-sync → done）+ `$KEY.codex-skipped.json`（仅 §③.1 codex-skip 路径——marker 随 stage 5 commit 入库收口） |
| 阶段 ⑤ `sprint($KEY): mark done` | **仅** `sprint-status.yaml`。一般情况 auto-sync 改动已经在 stage 5 commit 中带走，本步 STATUS=skip 跳过即可。 |
| 阶段 ⑤.5 `test($KEY): atdd + e2e (run-sprint stage 5.5)` | `test_artifacts/$KEY-test-result.json` OR `test_artifacts/skipped-$KEY-*.md`（sandbox graceful skip 路径）+ `<frontend_dir>/<e2e_test_subdir>/$KEY*.spec.ts`（如 atdd 改动；目录按 harness-project-config.yaml 配置，默认 `console-web/tests/e2e`）+ `sprint-status.yaml`（test_status 段更新）+ `deferred-work.md`（FU-Test-* 项追加）。环境受限 / 测试 fail 都不阻 stage ⑥；test_artifacts/ 全无产出时 STATUS=skip。 |
...
| `T3` `test($KEY): atdd red-phase scaffold` | `test_artifacts/$KEY.atdd-checklist.md` + `<frontend_dir>/<e2e_test_subdir>/$KEY*.spec.ts`（目录按 harness-project-config.yaml 配置，默认 `console-web/tests/e2e`）+ `sprint-status.yaml` |
...
**Spec-driven planning-artifacts 回写（issue #9，v0.1.39）**：偶发 case：chore / story spec 显式要求回写 `_bmad-output/planning-artifacts/` 下的规划文档（典型场景：forward-only remediation 把修正后的 AC 文本 / drift-registry 表回写 `epics.md`——spec source of truth 本身就是交付物的一部分）。这类路径默认命中 `OUT_OF_SCOPE_BMAD` halt（issue #5 守卫）；在 spec md 头部 frontmatter 加 `planning_artifacts:` YAML list 声明完整 repo-relative 路径即可放行：
...
`harness-commit.py` 读到该字段后把列出的路径（仅 `planning-artifacts/` 子树、必须 .md 后缀、禁 `..`）从 step 2.5 守卫里豁免，随本 commit 一起 stage，并在 STATUS=ok 输出 `PLANNING_ARTIFACT=<path>` 信息行。retro-fulfill stage 下 spec = `chore-retro-c<EPIC>-<CODE>-*.md`（优先按 sprint-status.yaml `chore_spec:` 字段解析，缺失时 code-first glob 兜底）。**约束**：① 仅声明 spec Tasks 真正要求的回写路径；brainstorming/ / research/ 等其它兄弟子树**不在**豁免范围（仍 halt）。② 白名单仅在允许 project code 的 stage（②/④/⑤/retro-fulfill）生效——其它 stage 命中 OUT_OF_SCOPE_BMAD 时按 GUIDANCE 单独 commit。
...
**过程 marker .json 自动剔除（issue #8，v0.1.39）**：dev subagent 在 sandbox 跳过检查时偶发多产 `<KEY>.maven-skipped.json` / `<KEY>.sandbox-skipped.json` 之类的过程 marker——内容与 dev-result.json 的 `checks="skip"` + `checks_skip_reasons` 完全冗余。harness-commit.py 对**显式枚举**的已知 marker tag（白名单，绝不泛化到任意 `*-skipped.json`——`<KEY>.codex-skipped.json` 是 schema 内 artifact 不可吞）自动 unstage + rm（仅 untracked；modified 永不自动删），输出 `AUTO_FIXED=process-marker ...` 行作为信息，不 halt。源头收敛：`harness-prompt-suffix.py 2/4` 已注入"禁止过程 marker 文件"段。白名单外的新 marker 变种仍走 `UNEXPECTED_ARTIFACT` halt——确认安全后再进白名单。
...
- 调度 dev：
  ...
  >
  > **sandbox 跳过只写 dev-result.json**：环境受限（无 mvn / java / docker 等）导致的跳过，**只**记录到上面 `checks.<name>: "skip"` + `checks_skip_reasons.<name>`（必要时加 deferred-work.md FU 条目）；**不要**另写 `<KEY>.maven-skipped.json` 之类的过程 marker 文件（issue #8——harness-commit 会自动剔除，徒增噪音）。
...
注意：`harness-commit.py 3` 与 `4` 都**不跑**——stage 3/4 在 skip 路径下没有产物。marker
文件的收口走 stage 5：`<KEY>.codex-skipped.json` 已纳入 `harness-commit.py 5` 的 expected
产物清单（stage 2/4 也放行，防续作场景），stage 5 commit 会把 marker 随本 story 收口入库，
sprint-status 的 dev_status 推进（review → done）照常由 stage 5 auto-sync 处理。事后
`/harness-zh:codex-catchup` 补跑时会把 marker 归档为 `<KEY>.codex-skipped.resolved.json`
并随 catchup 的 stage 4 commit 入库（详 codex-catchup.md §4.6/§4.7）。
...
- **commit row 5-fallback**：`python3 .claude/harness/scripts/harness-commit.py 5-fallback $KEY`。一般情况：上一步 auto-sync 已经把 sprint-status.yaml 带进 row 5 commit，此时 worktree 干净，脚本退出码 2（STATUS=skip），主 agent 跳过本步。仅当 row 5 commit 后 sprint-status.yaml 仍有变更时，脚本退出码 0，主 agent 跑 commit。
- **deferred-work 趋势输出（§L）**：commit 完成后跑下面这条 Bash 给用户一行可视化进度，零结构化卡点：

  ```bash
  # schema v1 唯一口径：grep_deferred_status.sh 按 [status:*] tag 输出 待消化/已消化/孤儿 三计数。
  # （旧版本这里还 grep "Resolved by Story X.Y" 等 legacy inline 后缀——schema v1 起该标记被
  #  pre-commit gate ② 禁产，统计恒为 0，已删除。）
  ...
  if STATUS_OUT=$(bash .claude/harness/scripts/grep_deferred_status.sh "$KEY" 2>/dev/null); then
      ...
      if [ -n "$SUMMARY" ]; then
          echo "deferred-work（story $SHORT）：$SUMMARY"
      else
          echo "deferred-work（story $SHORT）：无命中条目"
      fi
  else
      echo "deferred-work（story $SHORT）：报告生成失败"
  fi
  ```
...
1. **spawn run-test-sprint subagent**（subagent 内部跑 §0.1 env probe + §1 T1/T3/T4 + 各自 commit）：
   ...
   >
   > **任务追踪工具禁令（issue #9）**：禁止调用 TaskCreate / TaskUpdate / TaskList 等任务管理工具——sprint 任务清单由 run-sprint 主 orchestrator 独占维护。run-test.md §进度可视化 的 TaskCreate 要求在本调用路径（作为 stage 5.5 subagent 被调起）下**不适用**；你的进度只通过最终返回 message 汇报。
...
3. **5-5 sanity gate**（chore-harness-epic-4-orchestration-observations T2.1 决策）：跑 `python3 .claude/harness/scripts/harness-commit.py 5-5 $KEY`：
   ...
   - STATUS=ok（罕见 — back-compat 路径：例如 solo-dev 独立跑 `/harness-zh:run-test --story <key>` 中途中断、subagent 没走到 T3/T4 commit）→ 主 agent 用 SUGGEST_COMMIT_MSG commit；这是 stage 5.5 single commit 旧路径的兼容兜底
...
- 调度 retrospective：
  ...
  >
  > **格式契约（强制，先做）**：先 Read `.claude/harness/prompt-suffixes/bmad-retrospective-suffix.md` 并全程遵守。要点：
  > 1. 写 retro 文件**之前**先跑其 §Pre-retro self-audit 脚本：`bash .claude/harness/scripts/run_retro_self_audit.sh <EPIC-1>`（参数 = 上一 epic 编号；脚本失败不阻断——按该 suffix 的"hint 不阻断"约定继续，§2 表格手工填）。
  > 2. Action items 段**必须**按其 §2 Form 1 书写（每条一个 H3 heading：`### <code> — <标题>`），**忽略**上游 BMad SKILL §8 的默认编号列表范式。
  > 3. 每条 action item 必须声明 `**Category**: dev` 或 `**Category**: harness`。
  > 不合规形态（编号列表 / 纯数字表格等）会让 stage 6 commit 的 parser 三 Form 全 0 命中 → fail-loud halt；格式细节以该 suffix 文件为准。
  >
...
- 等待返回。
  - **瞬态 API 错误重试（issue #9 finding 4）**：retro subagent 偶发死于瞬态 infra 错误（如 `API Error: Overloaded`——注意区别于 §−1.a / §3 第一行的配额信号 `hit your limit` / `rate limit` / `usage limit` / `quota` / `reset` 时间提示，以那两处完整列表为准；配额信号走 §3 halt 模板选项 5）。此时若 `git status --porcelain` 显示 worktree 干净（无 partial 产物），直接重新 dispatch 一次同样的 retro subagent 即可，**不**走 halt；重试一次仍失败 → 按 §3 halt 模板停。worktree 不干净 → 不要自动重试（partial 产物需要 solo-dev 判断），按 §3 halt。
...
loop:
  if LOOP_SCOPE == "single-epic":
    # 用 next-in-epic 锚定目标 epic 自己的 backlog——**不要**用全局 next：目标 epic 之前的
    # 其它 epic 仍有 backlog 时，全局 next 会取到别的 epic 的 story → 一条不跑就 break 的
    # 静默 no-op（sprint-status.py docstring 明确本场景用 next-in-epic）
    next_key = `python3 .claude/harness/scripts/sprint-status.py next-in-epic $TARGET_EPIC`  # 退出码 1 = 该 epic 无 backlog
    if 该 epic 无 backlog:
      if `epic-retro-status $TARGET_EPIC` != "done":
        if `epic-all-done $TARGET_EPIC` 退出码 0:
          走 §0.C retro-only 路径触发 retrospective
        else:
          # "无 backlog" ≠ "epic 全 done"——该 epic 有 story 停在 review / in-progress（中断遗留）。
          # 不要对带半成品 story 的 epic 直接 retro（stage 6 会把 epic-N 强翻 done，污染状态机）
          halt（既有 §3 模板）：提示先 `/harness-zh:run --continue` 续作该 story，再重跑 `--epic` 触发 retro
      break
  else:
...
| 触发条件 | 处理 |
|---|---|
| 子 agent 返回包含 `hit your limit` / `rate limit` / `usage limit` / `quota` / `reset` 等配额耗尽信号 | halt 并使用配额耗尽专属 halt 模板（选项 5）：等待 reset 时间后再让用户决定是否重启 stage（这是文本扫描唯一保留的一类——见 §-1.b 说明）。**stage 3 例外**：阶段 ③ codex subagent 的 quota/auth 命中**不 halt**，按 §③.1 skip+marker 处理（catchup-friendly）。续作时主 agent 调 `harness-state.py --resume-prompt --stage <N>` 拿 deterministic prompt 段（harness-changelog 2026-05-03 §D），不要手工拼描述。 |
| 预期产物文件缺失或为空（每阶段"验收"列出的） | halt（不重新拉 agent，让人介入）。**stage ⑥ 瞬态 infra 错误例外**（issue #9 finding 4）：retro subagent 死于瞬态 API 错误（如 `API Error: Overloaded`，**非**本表第一行的配额信号）且 `git status --porcelain` 干净 → 允许按阶段 ⑥ "等待返回"注记重 dispatch 一次，不 halt；重试仍失败或 worktree 不干净才按本行 halt |
...
| 单条 story 的**主阶段（①-⑤）**累计调用 > 6 个子 agent（构成：①②③④⑤ 共 5 个 + 至多 1 次中断重启）。阶段 ⑤.5 run-test、⑥ retro、⑥.5 residue、§0.A.0 retro-fulfill 的子 agent **不计入**本预算，但各有自己的上限：⑤.5 每 story 至多 1 个、⑥ / ⑥.5 每 epic 各至多 1 个（各自另允许至多 1 次额外重启——配额中断重启**或** ⑥ 的瞬态 infra 错误重试，两者**合计**至多 1 次，即单 epic ⑥ 总 dispatch 上限 = 2）、§0.A.0 每个待兑现 dev 项至多 1 个——超过各自上限同样按本行 halt | halt |
...
**绝对禁止**的逃逸路径：
- 跳过任何阶段
- 把失败的子 agent 重新拉一次（代答政策重发不算；stage ⑥ retro 瞬态 infra 错误 + 干净 worktree 的单次重试不算——见阶段 ⑥ "等待返回"注记及上表预算行）
- 自动决定"小问题忽略"继续推进
