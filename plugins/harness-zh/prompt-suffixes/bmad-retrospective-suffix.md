...
>
> **加载机制**：`run.md` 阶段 ⑥ 的 retro dispatch prompt **显式要求** retro
> subagent 先 Read 本文件，Action items 段按 §"Action items markdown 格式契约"
> Form 1 书写、每条声明 `**Category**`，并先跑 §Pre-retro self-audit 脚本。
> 本文件不靠 skill 自动加载——注入指令固化在 run.md 阶段 ⑥ 的 dispatch
> prompt 模板里（§0.C retro-only 路径复用同一模板）。
...
输出 markdown 表格 4 列（id / 描述 / 自动判定 status / evidence）。retro
agent 把表格 paste 到 retro 文档 §2 cross-reference 作草稿基础，再增补
evidence / 调整 status / 写跨 epic 兑现度评估。

**hint 不阻断**：脚本失败（如新 epic 脚本 case 还没写完）不阻 retro 继续
——stderr WARN 一行后照常手工填 §2 表格，**不要 halt / 不要等人确认**。
每加新 epic action items 时同步更新脚本规则（在
`.claude/harness/scripts/run_retro_self_audit.sh` 加 `check_DN()` function）。
...
retro agent 在 paste 后必须为每行：
1. 复核 evidence 真实性（脚本是 grep 兜底，不替代判断）
2. 增补 commit hash / 实施日期 / 跨 epic 影响评估
3. 必要时调整 status（如脚本判 done 但实际 partial）
...
- **follow-through section 过滤**（v0.1.31 引入）：retro md 含
  `## Epic N retro Action items follow-through`（BMad SKILL Step 3
  prev-retro recap section）时，parser 跳过标题含 "follow-through" /
  "follow up" / "carryover" 任一关键字的 `## ` section——**两条解析路径
  都过滤**：① 找到 canonical §Action items section（标题含 "action item" /
  "行动项"）时，候选筛选阶段排除 follow-through section、优先取最后一个
  canonical section；② 找不到 canonical section、回落整文件兜底扫描时，
  同样先切除 follow-through section 再做 3 form 匹配。目的：避免把
  prev-epic AI 项错误 seed 进 current epic retro_action_items 块。
  **retro 作者侧仍要守约**：新 action items section 标题必须含
  "action item" 或 "行动项"（Form 1 契约本身的要求）——标题措辞漂移
  （如『改进计划』）会让 parser 落到兜底扫描路径，丧失 canonical
  section 定位，依赖的是过滤兜底而非一等公民路径。
...
1. 先按主题分组排序（process / tech-debt / test / doc / 自我约束 ...）——
   **仅用于组织条目顺序与措辞，不写进 `**Category**` 行**（机器只认
   dev|harness 两值，见 §6）
2. 每条 action 起一个 `### A<N> — <一句话>` H3 heading
3. heading 下面写：
   - `**Owner**: solo-dev`
   - `**Category**: dev` 或 `**Category**: harness`（仅此两值；机器解析，详 §6）
...
每条 action item 必须在 H3 heading 块内显式声明 `**Category**: dev` 或
`**Category**: harness`（取值**仅此两个**；§5 的 process/tech-debt/... 是
主题分组维度，不要写进 `**Category**` 行）。**v0.1.26 起两类都写
sprint-status.yaml**（不再分流到外部文件）。

**声明如何被机器消费**：stage ⑥ commit 时 `harness-commit.py` 的 seeder
解析每个 H3 块内的 `**Category**: dev|harness` 行，把 `category` 子键随条目
一并写入 sprint-status.yaml 的 `retro_action_items.epic-N-retro` 块。
**缺省 / 取值非法时不写 category 子键**——gate 端按 NOCAT 保守处理
（stderr WARN + 默认 harness，不阻 commit，无 halt）。stage ⑥.5 residue
流程的 MANIFEST 判定可为它处理的条目补 / 校正 category。所以：漏写
`**Category**` 不会断流水线，但 dev 类强约束（见下）在被补分类前**不生效**
——想让 gate 真拦，就把声明写对。

`category` 子键决定 pre-commit gate 行为：

- `category: dev` — 阻后续 epic spec 创建（pre-commit gate ①，强约束）
- `category: harness` — stderr WARN，不阻 commit；hint 用户用 `/harness-zh:report-issue` 提 issue
- 缺 category（NOCAT）— 视同 harness：stderr WARN，不阻 commit
...
retro 阶段产出 action items 时两类**写入路径相同**（都进
`_bmad-output/implementation-artifacts/sprint-status.yaml.retro_action_items.epic-N-retro` 块）。
**分工**：retro agent 只写 retro md（Form 1 + `**Category**` 声明）；yaml 块由
stage ⑥ commit 时 `harness-commit.py` seeder 按 md 解析结果自动写入（含
`category` 子键），`chore_spec` 子键由 stage ⑥.5 residue 流程补——retro
agent **不要手工编辑** sprint-status.yaml 的该块。落盘形态：

```yaml
retro_action_items:
  epic-N-retro:
    <CODE>: <status>          # 一句话描述（inline comment，seeder 写）
      category: dev | harness    # seeder 从 md **Category** 声明解析；缺省不写此键
      chore_spec: '<filename>'   # optional；stage ⑥.5 residue 流程补（仅 dev 类一般用）
```
...
（dev 期间漏 regen），retro 顺手 regen（用 Edit/Write 更新文件即可——
**不做 git 操作**，commit 由主 agent 的 stage ⑥ `harness-commit.py` 统一收口）。
