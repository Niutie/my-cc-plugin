...
Output (stdout): text block to append to a subagent prompt. Currently:
  ...
  - All stages: task-tools ban (issue #9 finding 2, v0.1.39) — subagents must
    not call TaskCreate/TaskUpdate/TaskList; the orchestrator's task list is
    not theirs to touch.
  - Stages 2 / 4: process-marker ban (issue #8, v0.1.39) — sandbox skips go
    into dev-result.json checks_skip_reasons, never a `<KEY>.*-skipped.json`
    marker file.
...
# Issue #9 finding 2 (v0.1.39) — subagents polluting the orchestrator's task
# list. Observed in an epic-6 run: stage-2 dev / stage-5 bmad-review /
# stage-5.5 run-test subagents flipped/created/deleted the MAIN agent's
# sprint tasks (at one point wiping the entire task list). Task tracking is
# the orchestrator's exclusive responsibility (run.md §0 (c)); subagents
# report progress only through the artifact channels their prompt defines.
TASK_TOOLS_BAN_BLOCK = """**任务追踪工具禁令（主 orchestrator 独占）**：禁止调用 TaskCreate / TaskUpdate / TaskList / TaskGet / TaskStop 等任务管理工具。sprint 任务清单由主 orchestrator 独占维护——子 agent 建任务 / 翻任务状态 / 删任务会直接污染主 agent 的调度清单（已发生过子 agent 清空整个任务清单的事故）。你的进度只通过本 prompt 规定的渠道汇报：story md checkbox / progress JSON / 最终返回 message。"""


# Issue #8 / issue #9 finding 3 (v0.1.39) — dev subagents spilling redundant
# process-marker files. Observed: `<KEY>.maven-skipped.json` recording
# "backend tests skipped, sandbox has no mvn" — information already fully
# captured by dev-result.json checks/checks_skip_reasons. harness-commit.py
# auto-prunes the known tags (AUTO_FIXED=process-marker) as a backstop; this
# block is the source-side convergence so the marker never gets written.
NO_PROCESS_MARKER_BLOCK = f"""**禁止过程 marker 文件**：sandbox / 环境受限导致的检查跳过（mvn / java / docker 不可用等）**只**记录到 `{_ARTIFACTS_REL}/$KEY.dev-result.json` 的 `checks.<name>: "skip"` + `checks_skip_reasons.<name>`（必要时再加 deferred-work.md 的 FU 条目）。**不要**另写 `$KEY.maven-skipped.json` / `$KEY.sandbox-skipped.json` 之类的过程 marker 文件——信息完全冗余，harness-commit.py 会自动剔除它们（`AUTO_FIXED=process-marker`），徒增 commit 噪音。"""
...
# Issue #8 — process-marker ban on the dev stages (2 = initial dev, 4 = codex
# fix round; both run /bmad-dev-story-style work that can hit sandbox skips).
NO_PROCESS_MARKER_STAGES = {"2", "4"}

# Issue #9 finding 2 — task-tools ban on every stage. The observed polluters
# were stages 2/4/5(+run-test), but any subagent with tool access can trash
# the orchestrator's task list — same all-stages posture as GIT_STATUS_STAGES.
TASK_TOOLS_BAN_STAGES = {"1", "2", "3", "4", "5", "6"}
...
def main():
    ...
    if stage in NO_PROCESS_MARKER_STAGES:
        blocks.append(NO_PROCESS_MARKER_BLOCK)
    if stage in TASK_TOOLS_BAN_STAGES:
        blocks.append(TASK_TOOLS_BAN_BLOCK)
